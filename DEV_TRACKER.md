...
**Model-agnostic `response_format` + prompted-JSON parsing [DONE — 2026-07-24]**: Made the whole optimizer work with any target model, whether or not it supports OpenAI-style `response_format`. Root cause found while running the live Nemotron E2E: LiteLLM's `get_supported_openai_params` reports `response_format` at the *provider* level, so it wrongly claimed `nvidia_nim/nvidia/llama-3.3-nemotron-super-49b-v1` supports it — the model 400s (`UnsupportedParamsError`) on the param at runtime. Fix: `registry.py` gains a curated `_MODELS_WITHOUT_JSON_MODE` override (same pattern as `_NO_KEY_PROVIDERS`) so `supports_json_mode` returns the truth, plus a `json_mode_params(model, schema)` helper. All structured-output call sites — `mutator.py` (mutate/critique/few-shot), `judge.py`, `rubric_generator.py` — now send `response_format` only where genuinely supported; unsupported models use the prompt's existing "respond with JSON only" instruction and are parsed free-form. `sweep.py` already gated on `supports_json_mode`, so it became correct for free. Second bug found live: models on the prompted-JSON path decorate output (markdown fences, preamble, a stray trailing brace) which broke strict `model_validate_json`. New `packages/core/src/reprompt_core/llm/json_extract.py`: lenient extractor that strips fences/preamble/trailing junk and returns the first balanced JSON value (respects strings/escapes); wired into all 5 parse sites. Removed the hardcoded `if model.startswith("nvidia_nim/")` hack from `test_e2e_nemotron.py` + `demo_nemotron_migration.py` — handling is now automatic and model-agnostic. Verified live against Nemotron: target, mutator, judge all pass individually; full `test_e2e_nemotron.py` passes (22 min — longer than the 13-min fallback run *because* the mutator now really generates variants). New tests: `test_json_extract.py` (11), `test_llm_registry.py` (+2 override/params tests). `packages/core`: **376 passed, 21 skipped**. `apps/api`: **227 passed** unchanged.

**E2E Test Pipeline [DONE — 2026-07-23]** : Real end-to-end migration harness using NVIDIA's free Nemotron model as the migration target. `nvidia_nim/nvidia/llama-3.1-nemotron-70b-instruct` added to `CURATED_MODELS` (`migrations.py`) and Tier 2 of `_GENERAL_ANALYSIS_TIERS` (`model_select.py`) — now selectable in the target model picker + auto-picked for judge/mutator. `live` pytest marker registered in `packages/core/pyproject.toml`. Provider-wiring gate tests added to `test_llm_registry.py` (3 new): `test_nvidia_nim_provider_name_is_nvidia_nim`, `test_nvidia_nim_requires_api_key`, `test_nvidia_nim_missing_credential_reports_nvidia_nim_api_key` — confirm LiteLLM routes `nvidia_nim/...` correctly and BYOK bridge will use the right env var. Two fixture files committed to `packages/core/tests/fixtures/query_log/`: `680d95a1-...txt` (3-stage CI fixture, fast) + `0f586e25-...txt` (32-stage demo fixture). New `packages/core/tests/test_e2e_nemotron.py` (`@pytest.mark.live`, skipped when `NVIDIA_NIM_API_KEY` absent): imports 3-stage fixture via existing `query_log` importer, builds `StageOptimizationInput`s (core path, no DB), runs `run_optimizer` with real Nemotron calls + $0.50 hard budget ceiling, asserts completion + `final_score ∈ [0,1]` + loose parity ≥ 0.3. Throttle wrapper (`_call_with_backoff`, 3-attempt exponential backoff) handles NVIDIA free-tier rate limits. `E2E_TARGET_MODEL` env var switches to `ollama/qwen2.5:14b` for keyless local runs. New `scripts/demo_nemotron_migration.py`: same core path, prints Gemini original cost vs optimizer spend, parity + winning prompt per stage — the "we migrated expensive Gemini → free Nemotron and parity held" demo story. New `docs/E2E_TESTING.md`: key/env var reference, run commands, rate-limit behavior, fixture table, parity score explanation. `packages/core`: **347 passed, 22 skipped** (3 new registry tests; live e2e test skipped when key absent). `apps/api`: **201 passed** unchanged. `apps/web`: **153 passed** unchanged.
