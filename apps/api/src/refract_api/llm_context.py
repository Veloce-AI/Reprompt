...
single :func:`refract_core.llm.client.complete` call.

The gap this module closes
---------------------------
``refract_core.llm.client.complete()`` deliberately never accepts a raw
...
(:class:`refract_api.models.WorkspaceApiKey`), not in the process
...
2. **Direct kwarg to LiteLLM — no env var involved at all.** LiteLLM's
   ...
   independent of the env-var convention. ``refract_core.llm.client.complete()``
...
``refract_core.llm.client.complete()`` — ``_scoped_api_key`` — rather than
...
from refract_core.llm.client import LLMResponse, Message, complete

from refract_api import models
from refract_api.crypto import decrypt
...
class ProviderKeyNotConfigured(Exception):
    ...
    Deliberately distinct from :class:`refract_core.llm.client.MissingAPIKeyError`
    ...
    ``refract_api.pipelines``'s ``/test-prompt`` endpoint) catch this
...
def resolve_workspace_credential(db: Session, workspace: models.Workspace, model: str) -> str:
    ...
    :class:`refract_api.crypto.EncryptionNotConfigured` (encryption key
...
def complete_with_workspace_credentials(
    ...
    **kwargs: Any,
    ...
    """:func:`refract_core.llm.client.complete`, scoped to ``workspace``'s
