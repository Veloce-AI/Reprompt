...
``reprompt-master-build-prompt.md`` §4: "email magic-link, single workspace
...
the server console) **only** when ``REPROMPT_DEV_MAGIC_LINKS`` is enabled
...
* Logging the link is gated behind the exact same flag as returning it in
  ...
  ``REPROMPT_DEV_MAGIC_LINKS=false`` (and wire in a real provider - not done
...
``REPROMPT_SESSION_SECRET`` must be set to a real secret before any real
...
WEB_BASE_URL = os.environ.get("REPROMPT_WEB_BASE_URL", "http://localhost:5173")
...
DEV_MODE_LINKS = _env_flag("REPROMPT_DEV_MAGIC_LINKS", default=True)

SESSION_SECRET = os.environ.get("REPROMPT_SESSION_SECRET")
if not SESSION_SECRET:
    SESSION_SECRET = "dev-insecure-session-secret-do-not-use-in-production"
    logger.warning(
        "REPROMPT_SESSION_SECRET is not set - falling back to an insecure "
        ...
        "via REPROMPT_SESSION_SECRET before any real deployment."
    )
...
def request_link(body: RequestLinkIn, db: Session = Depends(get_db)) -> RequestLinkOut:
    ...
    # configured and REPROMPT_DEV_MAGIC_LINKS is turned off.
