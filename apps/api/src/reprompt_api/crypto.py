...
-----------------------------------------------------------
`REPROMPT_SETTINGS_ENCRYPTION_KEY` must be set to a real Fernet key (32
...
Why not crash at startup: unlike `REPROMPT_SESSION_SECRET` (auth.py), which
...
ENV_VAR = "REPROMPT_SETTINGS_ENCRYPTION_KEY"


class EncryptionNotConfigured(RuntimeError):
    """Raised the moment encrypt()/decrypt() is called without a valid
    REPROMPT_SETTINGS_ENCRYPTION_KEY set - see module docstring for why this
...
def _fernet() -> Fernet:
    ...
    if not raw:
        raise EncryptionNotConfigured(
            f"{ENV_VAR} is not set. Reprompt encrypts BYOK provider API keys at "
            "rest and refuses to store or read them without a real encryption "
            "key (never a hardcoded or default one - see reprompt_api.crypto). "
            "Generate one with:\n"
...
def decrypt(ciphertext: str) -> str:
    ...
    reprompt_api.settings module docstring for the deferred runtime-injection
...
def reset_cache_for_tests() -> None:
    ...
    that monkeypatch REPROMPT_SETTINGS_ENCRYPTION_KEY between cases need the
