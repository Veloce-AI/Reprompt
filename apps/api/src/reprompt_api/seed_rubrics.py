...
``reprompt_core.rubric_generator.generate_rubric`` and
...
shaped exactly like ``reprompt_core.deterministic``'s check types - to build
...
env var reprompt_api.db reads) - e.g. to seed a pipeline already imported into
a running dev or e2e database:

    uv run python -m reprompt_api.seed_rubrics --pipeline-id 1
    uv run python -m reprompt_api.seed_rubrics --pipeline-name "Diamond Test Pipeline"
"""
...
def sample_deterministic_checks() -> list[dict]:
    ...
    Shape matches ``reprompt_core.deterministic.DeterministicCheck`` exactly:
...
def _main() -> None:
    ...
    # Imported lazily so DATABASE_URL (read at reprompt_api.db import time) is
