# Refract — Model Migration Parity Engine
...
**Stack:** Python FastAPI · Postgres · Celery + Redis · DSPy · LiteLLM · Optuna · Langfuse (self-host) · React + Tailwind. (Same stack as original Refract plan — nothing wasted.)
...
```
You are building "Refract", a model-migration parity engine. Read this
plan file fully before writing code: docs/refract-parity-engine-plan.md
...
2. Ingest command: `refract ingest <traces.json>` accepting a raw JSON
...
4. Name: "Refract" still fits (refracting a pipeline through a new model). Check domain/trademark before public repo.
