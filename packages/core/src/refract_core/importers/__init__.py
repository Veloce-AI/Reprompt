...
Per the plan doc's own stated philosophy (refract-parity-engine-plan.md §10.3):

    "Trace format: design your own canonical JSON, treat Langfuse/OTel as
    importers into it — don't marry Langfuse's schema."

``refract_core.trace`` (``TraceFile`` / ``parse_trace_file``) *is* that
...
see the YAGNI note below):
    ...
    def convert(raw: dict[str, Any]) -> dict[str, Any]:
        ...
        return a dict that validates via refract_core.trace.parse_trace_file
