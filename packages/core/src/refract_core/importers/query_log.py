...
and from ``refract_core.trace``'s shape, and both are handled here rather
...
def convert(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    Matches the :class:`refract_core.importers.ImporterFn` convention: takes
    one raw dict, returns a dict validate-able via
    :func:`refract_core.trace.parse_trace_file`. Does not validate itself —
