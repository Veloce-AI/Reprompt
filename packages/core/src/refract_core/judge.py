...
Per ``refract-parity-engine-plan.md`` §3 ("Judge: pairwise comparison
...
Every actual model call goes through :func:`refract_core.llm.client.complete`
...
``apps/api/src/refract_api/rubrics.py::JudgeCriterionIn`` and
``apps/api/src/refract_api/seed_rubrics.py::sample_judge_criteria``)::
...
:func:`refract_core.llm.client.complete` **twice** for the same
...
from refract_core.llm import client as llm_client
...
class JudgeResponseError(Exception):
    ...
    :mod:`refract_core.llm.client`'s transport-error taxonomy
    (:class:`~refract_core.llm.client.TransientLLMError` /
    ...
    *reaching* the model; this is about the model answering off-schema once
    reached. Errors raised by :func:`refract_core.llm.client.complete`
...
def judge_pairwise(
    ...
    *,
    ...
    Makes exactly two calls to :func:`refract_core.llm.client.complete` — see
    ...
    Raises whatever :func:`refract_core.llm.client.complete` raises
