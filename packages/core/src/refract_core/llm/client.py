...
Per ``refract-master-build-prompt.md`` working rule 6 ("Never hardcode API
...
any) it reads. See :mod:`refract_core.llm.registry` for the thin capability
...
from refract_core.llm.registry import missing_credential_env_vars, supports_json_mode
...
class RefractLLMError(Exception):
...
class TransientLLMError(RefractLLMError):
...
class PermanentLLMError(RefractLLMError):
...
class UnknownLLMError(RefractLLMError):
...
class LLMResponse:
    ...
    ``usage`` reuses :class:`refract_core.trace.TokenUsage` (the same
    ``tokens{in, out, thinking}`` shape a :class:`~refract_core.trace.StageRecord`
...
def complete(
    ...
    **extra_params: Any,
    ...
    response_format:
        ...
        (see :func:`refract_core.llm.registry.supports_json_mode`). Raises
