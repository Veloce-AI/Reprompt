...
Per ``refract-master-build-prompt.md`` §2 ("Model card: registry layer,
...
data)"), and ``refract-parity-engine-plan.md`` §3 ("...+ custom layer:
...
:mod:`refract_core.llm.registry` for cost/context-window/JSON-mode/
...
:func:`refract_core.llm.client.complete`.
...
def resolve_family(model: str) -> str:
    ...
    throughout :mod:`refract_core.llm.registry`.
