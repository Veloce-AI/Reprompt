...
Per ``refract-master-build-prompt.md`` §2 ("litellm for all model calls +
registry") and ``refract-parity-engine-plan.md`` §3/§6 ("Model cards:
...
and LLM judge, plus this package's own :mod:`refract_core.llm.client`).
...
def supports_json_mode(model: str) -> bool:
    ...
    Used both by :mod:`refract_core.llm.client` (to fail fast with a clear
