...
stage, per ``refract-parity-engine-plan.md`` §3(c)-(d).
...
* **simple** (default) — one mutation call
  (:func:`refract_core.optimizer.mutator.generate_prompt_mutations`), then
...
1. **Prompt mutation** (:mod:`refract_core.optimizer.mutator`) — propose a
   few rewritten prompt variants; degrades to the original prompt alone if
   mutation fails (never aborts the stage over this).
2. **Model-card transform** (:func:`refract_core.llm.model_card.apply_model_card_transform`)
...
4. **Param/format sweep** (:func:`refract_core.sweep.generate_param_format_grid`)
   ...
   budget hard-stop).
5. **Score** (:mod:`refract_core.scoring`, gated judge calls via
   :mod:`refract_core.judge`) and **select** (:func:`refract_core.selection.select_best_candidate`)
   the winner.
...
``on_attempt`` callback — ``apps/api/src/refract_api/optimizer_runner.py``
...
run (``refract-master-build-prompt.md``'s own M3 constraint).
"""
...
from refract_core.deterministic import DeterministicCheck, evaluate_deterministic_checks, parse_deterministic_checks
from refract_core.embedding import embedding_similarity
from refract_core.judge import judge_pairwise
from refract_core.llm.client import LLMResponse
from refract_core.llm.model_card import apply_model_card_transform
...
from refract_core.scoring import CompositeScore, compute_composite_score, score_candidate, should_run_judge
from refract_core.selection import ScoredSweepCandidate, select_best_candidate
from refract_core.sweep import SweepCandidate, generate_param_format_grid
...
def _apply_format_mode(prompt: str, format_mode: str) -> str:
    ...
    effect. ``refract_core.sweep`` deliberately only *generates* this
...
# PromptWizard backend (refract_core.optimizer.promptwizard). Only *how
...
def run_optimizer(
    ...
    *,
    ...
    call:
        ...
        (via :func:`refract_core.judge.judge_pairwise`, which calls
        :func:`refract_core.llm.client.complete` directly) judge calls.
    budget:
        Caller-owned :class:`~refract_core.budget.BudgetTracker` — real
