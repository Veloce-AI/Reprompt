...
:func:`refract_core.llm.client.complete` directly, or a closure wrapping
``refract_api.llm_context.complete_with_workspace_credentials``. Zero
...
class PromptMutationError(Exception):
    ...
    Distinct from :mod:`refract_core.llm.client`'s transport-error taxonomy
...
def _format_score_feedback(score: CompositeScore) -> str:
    ...
    directly from :class:`~refract_core.scoring.CompositeScore` rather than
...
def generate_prompt_mutations(
    ...
    *,
    ...
    call:
        ...
        :func:`refract_core.llm.client.complete` directly, or a closure
