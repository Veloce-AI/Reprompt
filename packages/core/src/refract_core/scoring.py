...
Per ``refract-parity-engine-plan.md`` §3 ("EVALUATION ENGINE: Score =
...
* the free/cheap-first orchestration (:func:`score_candidate`) that runs
  :mod:`refract_core.deterministic` and :mod:`refract_core.embedding`
  directly (both are free/local), and only spends judge tokens
  (:mod:`refract_core.judge`) when :func:`should_run_judge` says it's worth
  it.
...
from refract_core.embedding import DEFAULT_EMBEDDING_MODEL, embedding_similarity
...
def should_run_judge(
    ...
    *,
    ...
    ``refract_core.deterministic.CheckResult.reason``) when it's ``False``.
    """
...
def score_candidate(
    *,
    ...
    (:func:`refract_core.deterministic.evaluate_deterministic_checks`) and
    local embedding similarity
    (:func:`refract_core.embedding.embedding_similarity`) — then decides
    ...
    * Else (neither supplied), the judge term is simply omitted
    ...
    Raises ``ValueError`` from :func:`refract_core.embedding.embedding_similarity`
