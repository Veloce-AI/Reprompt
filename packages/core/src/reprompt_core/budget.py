...
Per ``reprompt-master-build-prompt.md`` §5 M3(d) ("budget accounting with
hard stop") and ``reprompt-parity-engine-plan.md``'s Migration model
(``budget`` — see ``apps/api/src/reprompt_api/models.py``'s
...
(:class:`reprompt_core.llm.client.LLMResponse`) is populated post-hoc, once
...
def filter_affordable_candidates(
    ...
    *,
    ...
    between :mod:`reprompt_core.sweep` (candidate generation) and this
    ...
    * ``is_valid=False`` candidates (pre-known-invalid — see
      ``reprompt_core.sweep``) are always excluded; they were never going
