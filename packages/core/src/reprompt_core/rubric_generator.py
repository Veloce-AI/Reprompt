...
``reprompt-parity-engine-plan.md`` §3 and M2 of ``reprompt-master-build-prompt.md``
...
than importing :func:`reprompt_core.llm.client.complete` itself, so a caller
can pass either ``complete`` directly (packages/core callers, CLI, tests) or
a closure around ``reprompt_api.llm_context.complete_with_workspace_credentials``
...
discriminated union) than :mod:`reprompt_core.deterministic`'s strict
discriminated-union :data:`~reprompt_core.deterministic.DeterministicCheck`
type. This mirrors how ``judge.py`` separates the LLM's raw per-criterion
output (:class:`~reprompt_core.judge._CriterionJudgment`) from the final
...
list via :func:`reprompt_core.deterministic.parse_deterministic_checks`
...
``apps/api/src/reprompt_api/rubrics.py`` and ``seed_rubrics.py``) with no
...
class StageOutputSample(BaseModel):
    ...
    ``StageRecord`` (see ``reprompt_core.trace.StageRecord`` /
    ``reprompt_api.models.StageRecord``) the generator actually needs.
...
class RubricGenerationResult(BaseModel):
    ...
    :mod:`reprompt_core.deterministic`'s real check types and dumped back to
    plain dicts (``by_alias=True`` — e.g. ``schema_`` -> ``"schema"`` —
    matching the exact shape ``reprompt_api.rubrics``/``seed_rubrics`` store
    in ``Rubric.deterministic_checks``). ``judge_criteria`` matches the
    ``{name, weight, description}`` shape already in use across
    ``reprompt_api.rubrics``/``seed_rubrics``/``reprompt_core.judge``.
    """
...
class RubricGenerationError(Exception):
    ...
    a separate error category from :mod:`reprompt_core.llm.client`'s
    transport-error taxonomy (:class:`~reprompt_core.llm.client.TransientLLMError` /
...
# concrete check `type` — see reprompt_core.deterministic for the canonical
...
def _translate_and_validate_checks(
    ...
    :mod:`reprompt_core.deterministic` types.
...
def generate_rubric(
    ...
    *,
    ...
    call:
        ...
        ``reprompt_api.llm_context.complete_with_workspace_credentials`` (see
    ...
    Whatever ``call`` itself raises
        (e.g. :class:`~reprompt_core.llm.client.MissingAPIKeyError`,
        :class:`~reprompt_api.llm_context.ProviderKeyNotConfigured`,
        :class:`~reprompt_core.llm.client.TransientLLMError`) propagates
