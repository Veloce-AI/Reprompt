...
Per ``reprompt-parity-engine-plan.md`` §3 ("d. Select best candidate >=
...
calls, no scoring) and does not know about :mod:`reprompt_core.budget`
...
this module guards against is a :class:`~reprompt_core.sweep.SweepCandidate`
...
see ``reprompt_core.sweep``'s module docstring). Such a candidate should
...
class ScoredSweepCandidate(BaseModel):
    """A :class:`~reprompt_core.sweep.SweepCandidate` paired with the
    :class:`~reprompt_core.scoring.CompositeScore` it received after being
...
def select_best_candidate(
    ...
    Implements ``reprompt-parity-engine-plan.md`` §3(d) literally:
    ...
    ValueError
        ...
        case: only valid candidates (per ``reprompt_core.sweep``) should
    ...
    if not eligible:
        raise ValueError(
            ...
            "reprompt_core.sweep.SweepCandidate.is_valid) should ever be executed and scored."
        )
