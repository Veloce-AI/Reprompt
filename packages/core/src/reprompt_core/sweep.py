...
Per ``reprompt-master-build-prompt.md`` §2 (stack: "optuna (param sweeps,
grid sampler for MVP)") and §5 M3(c) ("Optuna grid sweep over temperature x
format (XML/JSON/plain) x structured-output mode") and
``reprompt-parity-engine-plan.md`` §7 ("Cut from MVP: ... Optuna (grid sweep
...
:func:`reprompt_core.llm.client.complete`) and scoring it
(:mod:`reprompt_core.scoring`) are the M3 optimizer loop's job, a later,
...
1. ``structured_output_mode=True`` requested against a model that doesn't
   ...
   :func:`reprompt_core.llm.registry.supports_json_mode`).
...
:func:`reprompt_core.budget.filter_affordable_candidates`, which does this
...
from reprompt_core.llm.registry import supports_json_mode
from reprompt_core.trace import FormatMode
...
``reprompt_core.trace.StageParams``) is a superset that also allows
...
``response_format``, see :func:`reprompt_core.llm.registry.supports_json_mode`)."""
...
class SweepCandidate(BaseModel):
    ...
    :mod:`reprompt_core.selection`) varies across a run.
    """
    ...
    *,
    ...
    if structured_output_mode and not model_supports_json_mode:
        return False, (
            ...
            "(reprompt_core.llm.registry.supports_json_mode reports False for this model)."
        )
    return True, None
...
def generate_param_format_grid(
    ...
    *,
    ...
    model:
        ...
        via :func:`reprompt_core.llm.registry.supports_json_mode`) for the
    ...
    study = optuna.create_study(sampler=sampler, study_name=f"reprompt-param-format-sweep-{model}")
